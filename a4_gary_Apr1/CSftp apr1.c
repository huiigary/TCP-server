...
void handleRETR(char* inputFileName, int fd, int dataSocket) {// RETR -- gets a file from the server AND transfer to client side
    ...
    if(targetFile == NULL){
     ...
     while (1) {
      ...
      int bytes_read = fread(buffer, 1, sizeof(buffer), targetFile);// BUFFER = 1024 bytes READ from targetfile
    ...
    while (bytes_read > 0) {
        ...
        bytes_read -= bytes_written; // we pass bytes_read int write bytes.
        p += bytes_written;
    }
}
     printf("%s \r\n","exited while loop of sending bytes");
     close(dataSocket);
     return;
}
...
int goodbind=1;
  ...
  for(p = servinfo; p != NULL; p = p->ai_next) { // P = THE LINKED LIST OF structInfo
    ...
    // reach here --> goodbind = 0 bc we have a good bind
    goodbind=0;
    ...
    break;
     ...
     // get the CURRENT ADDRESS the socketFD is bound, pointed to by addr
    // addrlen = length pointed to by addr
    ...
    printf("%s", "---- socket was properly created-----\r\n");
  ...
  if (sigaction(SIGCHLD, &sa, NULL) == -1) {
    ...
    handlepassivemode(fd, localIP, myp1, myp2);  //todo
...
void handle_client(int fd) { 
    ...
    getcwd(ftp_begin_dir,MAX_LINE_LENGTH + 1);      // get the current Working directory


    net_buffer_t recvBuf = nb_create(fd, MAX_LINE_LENGTH);
    char buf[MAX_LINE_LENGTH + 1] = {0};
    send_string(fd, "220 Please Login\r\n");
    int val = 1;
    ...
    while (val == 1) {
        ...
        while (login == 1) { //while we are logged into the correct user we have access to all implemented commands
            ...
            int readSize=nb_read_line(recvBuf, buf);
            if( readSize ==-1){
                printf("%s", "error in read_line\r\n");
            }
            //  printf("read_line= %i\r\n", nb_read_line(recvBuf, buf)); // THIS BLOCKS
            //  printf("read_line= %i\r\n", readSize); 
            ...
            if ((strcasecmp(command, "USER") == 0)) { //allows user to re-login
                ...
                if (input_string_1 == NULL){ // Quit if no subcommandscwcwccs
                    val = 0;
                    login = 0;
                ...
                if (input_string_1 == NULL || input_string_2 != NULL){
                   ...
                   // send_string(fd, "%s\r\n", getcwd(cwd,MAX_LINE_LENGTH + 1)); //print current directory
                    ...
                    memset(cwd,0,MAX_LINE_LENGTH + 1);
                    getcwd(cwd,MAX_LINE_LENGTH + 1);
                    send_string(fd, "%s\r\n",getcwd(cwd,MAX_LINE_LENGTH + 1)); //print NEW current directory
                    printf("%s -- new Dir \r\n",getcwd(cwd,MAX_LINE_LENGTH + 1) );
                ...
                if(input_string_1 != NULL){
                    ...
                    if (strcmp(cwd,ftp_begin_dir) == 0){
                        ...
                        char *last_backslash = strrchr(trimwhitespace(cwd), '/');//get pointer to last occurrence of  "/"
                        int length_curr_dir = strlen(trimwhitespace(cwd));// length of current directory path
                        ...
                        strncpy(dest,trimwhitespace(cwd),size_substract);

                        chdir(trimwhitespace(dest));//change dir to parent
                ...
                if (dataSocket == -1){
                        send_string(fd, "425 - Can't open data connection\r\n");
                }else {
                ...
                if (input_string_1 != NULL){
                   ...
                   // start: socket is not set
                   // then : when we use it ,--> becomes some #
                    ...
                    if (dataSocket != -1){  // close the socket if its in use
                     ...
                     if (dataSocket == -1){
                                         printf("%s\r\n","in -1 code");
                        dataSocket = handlePASV(fd);    // COMMENTED OUT bc it didnt work when i did "NLIST 2x IN a row"
                    }
                printf("%s\r\n","in handlepasv");
                    dataSocket = handlePASV(fd); 
            ...
            else { send_string(fd, "500 - Syntax error, command unrecognized\r\n"); // Syntax error, command unrecognized OR not implemented
